...
from MetaTask_metasp import MetaTaskDataLoader
from MetaLearner_metasp import Reptile, MAML
from utils import *
import argparse
import torch
...
def main():
    ...
    parser.add_argument("--k_spt_label", default=8, type=int,
                        help="Number of label samples in support set")
    
    parser.add_argument("--k_spt_unlabel", default=8, type=int,
                        help="Number of unlabel samples in support set")

    parser.add_argument("--k_qry", default=8, type=int,
                        help="Number of label samples in query set")
    ...
    parser.add_argument("--mlm_lambda", default=0.1, type=float,
                        help="Trade-off between mlm loss and prompt loss")
    ...
    domains = ['books', 'dvd', 'electronics', 'kitchen_housewares']
    domains = ['apparel', 'baby', 'books', 'camera_photo', 'dvd', 'electronics', 'health_personal_care', 'imdb', 'kitchen_housewares', 'magazines', 'MR', 'music', 'software', 'sports_outdoors', 'toys_games', 'video']
    test_domains = ['apparel', 'baby', 'books', 'camera_photo', 'dvd', 'electronics', 'health_personal_care',
                    'imdb', 'kitchen_housewares', 'magazines', 'MR', 'music', 'software', 'sports_outdoors', 'toys_games', 'video']
    ...
    for testDomain in test_domains:
        ...
        for trainDomain in trainDomains:
            ...
            train_dataset[trainDomain]['label'] = []
            train_dataset[trainDomain]['unlabel'] = dataset[trainDomain]['unlabel']
            
            train_dataset[trainDomain]['label'].extend(
        ...
        prompt_model = PromptForClassification(
            plm=plm, template=mytemplate, verbalizer=myverbalizer, freeze_plm=(not args.tune_plm), plm_eval_mode=False)
        print(prompt_model)

        ####### wandb section
        run_name = f'Meta test on {testDomain}'
        job_type = f'fasp'
        ...
        wandb.init(project='fsUDA', name=run_name, config=config,
        ...
        trainLoader = MetaTaskDataLoader(
            ...
            num_tasks=args.num_task,
            k_spt_label=args.k_spt_label,
            k_spt_unlabel=args.k_spt_unlabel,
            k_qry=args.k_qry,
            shuffle=True,
        ...
        learner = Reptile(
            model=prompt_model, outer_update_lr=args.outer_lr, inner_update_lr=args.inner_lr, inner_update_step=args.inner_step, mlm_lambda = args.mlm_lambda, tune_plm=args.tune_plm, device=device)
        #learner = MAML(
        ...
        for epoch in range(args.epoch):
            ...
            if test_acc > best_acc:
                ...
                model_save_name = f'fasp_epoch{epoch}_acc{round(test_acc, 4)}.pt'
